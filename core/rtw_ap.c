...
static void update_BCNTIM(struct _ADAPTER *padapter)
{
	struct sta_priv *pstapriv = &padapter->stapriv;
	struct mlme_ext_priv *pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info *pmlmeinfo = &(pmlmeext->mlmext_info);
...
static void update_bmc_sta(struct _ADAPTER *padapter)
{
	_irqL	irqL;
	uint32_t	 init_rate=0;
...
static void update_hw_ht_param(struct _ADAPTER *padapter)
{
...
static void start_bss_network(struct _ADAPTER *padapter, uint8_t *pbuf)
{
	uint8_t *p;
...
static int rtw_ap_set_key(struct _ADAPTER *padapter, uint8_t *key, uint8_t alg, int keyid, uint8_t set_tx)
{
	uint8_t keylen;
	struct cmd_obj* pcmd;
...
static void update_bcn_erpinfo_ie(struct _ADAPTER *padapter)
{
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
...
static void update_bcn_wps_ie(struct _ADAPTER *padapter)
{
...
static void update_bcn_vendor_spec_ie(struct _ADAPTER *padapter, uint8_t *oui)
{
	DBG_871X("%s\n", __FUNCTION__);
...
*/
static int rtw_ht_operation_update(struct _ADAPTER *padapter)
{
...
uint8_t ap_free_sta(struct _ADAPTER *padapter, struct sta_info *psta, bool active, uint16_t reason)
{
	_irqL irqL;
	uint8_t beacon_updated = _FALSE;
	struct mlme_priv *pmlmepriv = &(padapter->mlmepriv);
