...
-----------------------------------------------------------------------------*/
uint8_t *rtw_get_ie(uint8_t *pbuf, sint index, sint *len, sint limit)
{
	sint tmp,i;
	uint8_t *p;
_func_enter_;
...
_func_enter_;
	...
	*(uint16_t *)ie = cpu_to_le16((uint16_t)pdev_network->Configuration.BeaconPeriod);//BCN_INTERVAL;
	sz += 2;
	ie += 2;
	...
	*(uint16_t *)ie |= cpu_to_le16(cap_IBSS);
...
int rtw_get_wapi_ie(uint8_t *in_ie,uint in_len,uint8_t *wapi_ie,uint16_t *wapi_len)
{
	uint8_t authmode, i;
	uint 	cnt;
	uint8_t wapi_oui1[4]={0x0,0x14,0x72,0x01};
	uint8_t wapi_oui2[4]={0x0,0x14,0x72,0x02};
...
uint8_t *rtw_get_wps_attr(uint8_t *wps_ie, uint wps_ielen, uint16_t target_attr_id ,uint8_t *buf_attr, uint32_t	 *len_attr)
{
	...
	while(attr_ptr - wps_ie < wps_ielen)
	{
		...
		uint16_t attr_id = RTW_GET_BE16(attr_ptr);
		uint16_t attr_data_len = RTW_GET_BE16(attr_ptr + 2);
...
uint8_t *rtw_get_p2p_ie(uint8_t *in_ie, int in_len, uint8_t *p2p_ie, uint *p2p_ielen)
{
	uint cnt = 0;
	uint8_t *p2p_ie_ptr;
	uint8_t eid, p2p_oui[4]={0x50,0x6F,0x9A,0x09};
...
uint8_t *rtw_get_p2p_attr(uint8_t *p2p_ie, uint p2p_ielen, uint8_t target_attr_id ,uint8_t *buf_attr, uint32_t	 *len_attr)
{
	uint8_t *attr_ptr = NULL;
	uint8_t *target_attr_ptr = NULL;
	uint8_t p2p_oui[4]={0x50,0x6F,0x9A,0x09};
	...
	while(attr_ptr - p2p_ie < p2p_ielen)
	{
		...
		uint16_t attr_data_len = RTW_GET_LE16(attr_ptr + 1);
		uint16_t attr_len = attr_data_len + 3;
...
uint32_t	 rtw_set_p2p_attr_content(uint8_t *pbuf, uint8_t attr_id, uint16_t attr_len, uint8_t *pdata_attr)
{
	...
	*pbuf = attr_id;
	...
	//*(uint16_t *)(pbuf + 1) = cpu_to_le16(attr_len);
...
static uint rtw_p2p_attr_remove(uint8_t *ie, uint ielen_ori, uint8_t attr_id)
{
	...
	while(1) {
		...
		{
			uint8_t *next_attr = target_attr+target_attr_len;
...
void rtw_WLAN_BSSID_EX_remove_p2p_attr(WLAN_BSSID_EX *bss_ex, uint8_t attr_id)
{
	...
	if( (p2p_ie=rtw_get_p2p_ie(bss_ex->IEs+_FIXED_IE_LENGTH_, bss_ex->IELength-_FIXED_IE_LENGTH_, NULL, &p2p_ielen_ori)) )
	{
		...
		if(p2p_ielen != p2p_ielen_ori) {
			...
			uint8_t *next_ie_ori = p2p_ie+p2p_ielen_ori;
			uint8_t *next_ie = p2p_ie+p2p_ielen;
...
int rtw_get_wfd_ie(uint8_t *in_ie, int in_len, uint8_t *wfd_ie, uint *wfd_ielen)
{
	int match;
	uint cnt = 0;
	uint8_t eid, wfd_oui[4]={0x50,0x6F,0x9A,0x0A};
...
int rtw_get_wfd_attr_content(uint8_t *wfd_ie, uint wfd_ielen, uint8_t target_attr_id ,uint8_t *attr_content, uint *attr_contentlen)
{
	int match;
	uint cnt = 0;
	uint8_t attr_id, wfd_oui[4]={0x50,0x6F,0x9A,0x0A};
	...
	while( cnt < wfd_ielen )
	{
		uint16_t attrlen = RTW_GET_BE16(wfd_ie + cnt + 1);
