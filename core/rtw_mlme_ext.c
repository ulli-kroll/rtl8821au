...
static uint8_t init_channel_set(_adapter* padapter, uint8_t ChannelPlan, RT_CHANNEL_INFO *channel_set)
{
...
static uint8_t cmp_pkt_chnl_diff(_adapter *padapter,uint8_t * pframe,uint packet_len)
...
#ifdef CONFIG_P2P
uint32_t	 p2p_listen_state_process(_adapter *padapter, unsigned char *da)
{
	bool response = _TRUE;
...
_continue:
			...
			pmlmepriv->cur_network.join_res == _TRUE)
	{
		...
		uint8_t *mac_addr, *peer_addr;
		struct sta_priv *pstapriv = &padapter->stapriv;
		uint8_t RC_OUI[4]={0x00,0xE0,0x4C,0x0A};
		...
		if (psta == NULL)
		{
			...
			psta->pid = (uint16_t)(((mac_addr[4]<<8) + mac_addr[5]) + ((peer_addr[4]<<8) + peer_addr[5]));
...
unsigned int OnAssocReq(_adapter *padapter, union recv_frame *precv_frame)
{
...
#ifdef CONFIG_P2P
	struct wifidirect_info	*pwdinfo = &(padapter->wdinfo);
	uint8_t p2p_status_code = P2P_STATUS_SUCCESS;
	uint8_t *p2pie;
	uint32_t	 p2pielen = 0;
#ifdef CONFIG_WFD
	...
	if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO))
	{
		if( (p2pie=rtw_get_p2p_ie(pframe + WLAN_HDR_A3_LEN + ie_offset , pkt_len - WLAN_HDR_A3_LEN - ie_offset , NULL, &p2pielen)))
		{
			...
			if((p2p_status_code=(uint8_t)process_assoc_req_p2p_ie(pwdinfo, pframe, pkt_len, pstat))>0)
			{
...
unsigned int on_action_spct_ch_switch(_adapter *padapter, struct sta_info *psta, uint8_t *ies, uint ies_len)
{
...
static void get_channel_cnt_24g_5gl_5gh(  struct mlme_ext_priv *pmlmeext, uint8_t * p24g_cnt, uint8_t * p5gl_cnt, uint8_t * p5gh_cnt )
{
	int	i = 0;
...
void issue_p2p_GO_request(_adapter *padapter, uint8_t * raddr)
{
	...
	uint8_t			oui_subtype = P2P_GO_NEGO_REQ;
	uint8_t			wpsie[ 255 ] = { 0x00 }, p2pie[ 255 ] = { 0x00 };
	uint8_t			wpsielen = 0, p2pielen = 0, i;
	uint8_t			channel_cnt_24g = 0, channel_cnt_5gl = 0, channel_cnt_5gh = 0;
	uint16_t			len_channellist_attr = 0;
#ifdef CONFIG_WFD
	uint32_t					wfdielen = 0;
#endif //CONFIG_WFD
	...
	*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0001 );
	...
	//	Device Password ID
	//	Type:
	*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_DEVICE_PWID );
	wpsielen += 2;
	...
	*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0002 );
	...
	{
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_DPID_USER_SPEC );
	}
	else if ( pwdinfo->ui_got_wps_info == P2P_GOT_WPSINFO_SELF_DISPLAY_PIN )
	{
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_DPID_REGISTRAR_SPEC );
	}
	else if ( pwdinfo->ui_got_wps_info == P2P_GOT_WPSINFO_PBC )
	{
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_DPID_PBC );
	}
	...
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0x0002 );
	...
	//	Length:
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0x0001 );
	p2pielen += 2;
	...
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0x0002 );
	...
	//	Length:
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0x0005 );
	p2pielen += 2;
	...
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0xFFFF );
	...
	//	Length:
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( ETH_ALEN );
	p2pielen += 2;
	...
	len_channellist_attr = 3
	   + (1 + 1) * (uint16_t)(pmlmeext->channel_list.reg_classes)
	   + get_reg_classes_full_count(pmlmeext->channel_list);

#ifdef CONFIG_CONCURRENT_MODE
	if ( check_buddy_fwstate(padapter, _FW_LINKED ) )
	{
		*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 5 + 1 );
	}
	else
	{
		*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( len_channellist_attr );
	}
#else
...
void issue_p2p_GO_response(_adapter *padapter, uint8_t * raddr, uint8_t * frame_body,uint len, uint8_t result)
{

	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	uint8_t			action = P2P_PUB_ACTION_ACTION;
	uint32_t			p2poui = cpu_to_be32(P2POUI);
	uint8_t			oui_subtype = P2P_GO_NEGO_RESP;
	uint8_t			wpsie[ 255 ] = { 0x00 }, p2pie[ 255 ] = { 0x00 };
	uint8_t			p2pielen = 0, i;
	uint			wpsielen = 0;
	uint16_t			wps_devicepassword_id = 0x0000;
	uint			wps_devicepassword_id_len = 0;
	uint8_t			channel_cnt_24g = 0, channel_cnt_5gl = 0, channel_cnt_5gh;
	uint16_t			len_channellist_attr = 0;
	...
	*(fctrl) = 0;
	...
	rtw_get_wps_attr_content( wpsie, wpsielen, WPS_ATTR_DEVICE_PWID, (uint8_t *) &wps_devicepassword_id, &wps_devicepassword_id_len);
	...
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_be16( pwdinfo->device_name_len );
	...
	if ( rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) )
	{
		...
		//	Length:
		*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( ETH_ALEN + pwdinfo->nego_ssidlen );
		p2pielen += 2;
...
void issue_p2p_GO_confirm(_adapter *padapter, uint8_t * raddr, uint8_t result)
{

	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	uint8_t			action = P2P_PUB_ACTION_ACTION;
	uint32_t			p2poui = cpu_to_be32(P2POUI);
	uint8_t			oui_subtype = P2P_GO_NEGO_CONF;
	uint8_t			wpsie[ 255 ] = { 0x00 }, p2pie[ 255 ] = { 0x00 };
	uint8_t			wpsielen = 0, p2pielen = 0;
	...
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0x0005 );
	...
	//	Length:
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( pwdinfo->channel_list_attr_len );
	p2pielen += 2;
...
void issue_p2p_invitation_request(_adapter *padapter, uint8_t * raddr )
{
	...
	uint8_t			action = P2P_PUB_ACTION_ACTION;
	uint32_t			p2poui = cpu_to_be32(P2POUI);
	uint8_t			oui_subtype = P2P_INVIT_REQ;
	uint8_t			p2pie[ 255 ] = { 0x00 };
	uint8_t			p2pielen = 0, i;
	uint8_t			dialogToken = 3;
	uint8_t			channel_cnt_24g = 0, channel_cnt_5gl = 0, channel_cnt_5gh = 0;
	uint16_t			len_channellist_attr = 0;
#ifdef CONFIG_WFD
	uint32_t					wfdielen = 0;
	...
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( len_channellist_attr );
	...
	//	Length:
	*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 6 + pwdinfo->invitereq_info.ssidlen );
	p2pielen += 2;
...
void issue_p2p_invitation_response(_adapter *padapter, uint8_t * raddr, uint8_t dialogToken, uint8_t status_code)
{

	unsigned char category = RTW_WLAN_CATEGORY_PUBLIC;
	uint8_t			action = P2P_PUB_ACTION_ACTION;
	uint32_t			p2poui = cpu_to_be32(P2POUI);
	uint8_t			oui_subtype = P2P_INVIT_RESP;
	uint8_t			p2pie[ 255 ] = { 0x00 };
	uint8_t			p2pielen = 0, i;
	uint8_t			channel_cnt_24g = 0, channel_cnt_5gl = 0, channel_cnt_5gh = 0;
	uint16_t			len_channellist_attr = 0;
...
void issue_p2p_provision_request(_adapter *padapter, uint8_t * pssid, uint8_t ussidlen, uint8_t * pdev_raddr )
{
	...
	uint32_t			p2poui = cpu_to_be32(P2POUI);
	uint8_t			oui_subtype = P2P_PROVISION_DISC_REQ;
...
uint8_t is_matched_in_profilelist( uint8_t * peermacaddr, struct profile_info* profileinfo )
{
	uint8_t i, match_result = 0;
...
void issue_probersp_p2p(_adapter *padapter, unsigned char *da)
{
	...
	uint16_t					capInfo = 0;
	struct wifidirect_info	*pwdinfo = &(padapter->wdinfo);
	uint8_t					wpsie[255] = { 0x00 };
	uint32_t					wpsielen = 0, p2pielen = 0;
#ifdef CONFIG_WFD
	uint32_t					wfdielen = 0;
#endif //CONFIG_WFD
#ifdef CONFIG_IOCTL_CFG80211
	...
	uint8_t listen_channel = (uint8_t) ieee80211_frequency_to_channel(ieee_ch->center_freq);
#endif //CONFIG_IOCTL_CFG80211
	...
	*(fctrl) = 0;
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0001 );
		...
		{
			//Sec dev type
			*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_SEC_DEV_TYPE_LIST );
			wpsielen += 2;

			//	Length:
			*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0008 );
			wpsielen += 2;

			//	Value:
			//	Category ID
			*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_PDT_CID_DISPLAYS );
			wpsielen += 2;

			//	OUI
			*(uint32_t *) ( wpsie + wpsielen ) = cpu_to_be32( INTEL_DEV_TYPE_OUI );
			wpsielen += 4;

			*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_PDT_SCID_WIDI_CONSUMER_SINK );
			wpsielen += 2;
		...
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_SIMPLE_CONF_STATE );
		wpsielen += 2;
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0001 );
		...
		//	Response Type
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_RESP_TYPE );
		wpsielen += 2;
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0001 );
		...
		//	UUID-E
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_UUID_E );
		wpsielen += 2;

		//	Length:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0010 );
		wpsielen += 2;
		...
		//	Manufacturer
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_MANUFACTURER );
		wpsielen += 2;

		//	Length:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0007 );
		wpsielen += 2;
		...
		//	Model Name
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_MODEL_NAME );
		wpsielen += 2;

		//	Length:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0006 );
		wpsielen += 2;
		...
		//	Model Number
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_MODEL_NUMBER );
		wpsielen += 2;
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0001 );
		...
		//	Serial Number
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_SERIAL_NUMBER );
		wpsielen += 2;

		//	Length:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( ETH_ALEN );
		wpsielen += 2;
		...
		//	Primary Device Type
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_PRIMARY_DEV_TYPE );
		wpsielen += 2;
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0008 );
		...
		//	Value:
		//	Category ID
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_PDT_CID_MULIT_MEDIA );
		wpsielen += 2;

		//	OUI
		*(uint32_t *) ( wpsie + wpsielen ) = cpu_to_be32( WPSOUI );
		wpsielen += 4;

		//	Sub Category ID
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_PDT_SCID_MEDIA_SERVER );
		wpsielen += 2;

		//	Device Name
		//	Type:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_ATTR_DEVICE_NAME );
		wpsielen += 2;

		//	Length:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( pwdinfo->device_name_len );
		wpsielen += 2;
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0002 );
		...
		//	Value:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( pwdinfo->supported_wps_cm );
		wpsielen += 2;
...
int _issue_probereq_p2p(_adapter *padapter, uint8_t *da, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
	...
	struct wifidirect_info	*pwdinfo = &(padapter->wdinfo);
	uint8_t					wpsie[255] = { 0x00 }, p2pie[ 255 ] = { 0x00 };
	uint16_t					wpsielen = 0, p2pielen = 0;
#ifdef CONFIG_WFD
	uint32_t					wfdielen = 0;
#endif //CONFIG_WFD
	...
	*(fctrl) = 0;
...
#endif //CONFIG_IOCTL_CFG80211
	{
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0008 );
		...
		//	Value:
		//	Category ID
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_PDT_CID_RTK_WIDI );
		wpsielen += 2;
		...
		*(uint32_t *) ( wpsie + wpsielen ) = cpu_to_be32( WPSOUI );
		...
		//	Sub Category ID
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_PDT_SCID_RTK_DMP );
		wpsielen += 2;
		...
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( 0x0002 );
		...
		//	Value:
		*(uint16_t *) ( wpsie + wpsielen ) = cpu_to_be16( WPS_DPID_REGISTRAR_SPEC );	//	Registrar-specified
		wpsielen += 2;
...
inline void issue_probereq_p2p(_adapter *adapter, uint8_t *da)
{
...
unsigned int on_action_public_p2p(union recv_frame *precv_frame)
{
	...
	uint8_t	empty_addr[ETH_ALEN] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
#endif //CONFIG_P2P
		...
		switch( frame_body[ 6 ] )//OUI Subtype
		{
			...
			case P2P_INVIT_REQ:
			{
				...
				if ( (p2p_ie=rtw_get_p2p_ie( frame_body + _PUBLIC_ACTION_IE_OFFSET_, len - _PUBLIC_ACTION_IE_OFFSET_, NULL, &p2p_ielen)) )
				{
					...
					uint32_t	attr_contentlen = 0;
					uint8_t	status_code = P2P_STATUS_FAIL_INFO_UNAVAILABLE;
					struct group_id_info group_id;
					uint8_t	invitation_flag = 0;
					...
					if ( attr_contentlen )
					{
						...
						if ( invitation_flag & P2P_INVITATION_FLAGS_PERSISTENT )
						{
							...
							rtw_get_p2p_attr_content( p2p_ie, p2p_ielen, P2P_ATTR_GROUP_ID, ( uint8_t * ) &group_id, &attr_contentlen);
							if ( attr_contentlen )
							{
								...
								else
								{
									...
									if ( is_matched_in_profilelist( pwdinfo->p2p_peer_interface_addr, &pwdinfo->profileinfo[ 0 ] ) )
									{
										...
										{
											if( rtw_ch_set_search_ch(padapter->mlmeextpriv.channel_set, (uint32_t)operatingch_info[4] ) )
											{
...
#ifdef CONFIG_P2P
	uint8_t *frame_body;
	uint8_t category, OUI_Subtype, dialogToken=0;
	uint8_t *pframe = precv_frame->u.hdr.rx_data;
	uint len = precv_frame->u.hdr.len;
	struct	wifidirect_info	*pwdinfo = &( padapter->wdinfo );
...
void issue_beacon(_adapter *padapter, int timeout_ms)
{
	...
	*(fctrl) = 0;
	...
	if( (pmlmeinfo->state&0x03) == WIFI_FW_AP_STATE)
	{
		...
		uint32_t	 wpsielen=0, insert_len=0;
		uint8_t *wpsie=NULL;
		...
		if(rtw_p2p_chk_role(pwdinfo, P2P_ROLE_GO) && wpsie && wpsielen>0)
		{
			...
			uint8_t *premainder_ie, *pframe_wscie;
...
#endif //CONFIG_IOCTL_CFG80211
			{
				...
				//	Primary Device Type
				//	Type:
				*(uint16_t *) ( pframe + insert_len) = cpu_to_be16( WPS_ATTR_PRIMARY_DEV_TYPE );
				insert_len += 2;

				//	Length:
				*(uint16_t *) ( pframe + insert_len ) = cpu_to_be16( 0x0008 );
				insert_len += 2;

				//	Value:
				//	Category ID
				*(uint16_t *) ( pframe + insert_len ) = cpu_to_be16( WPS_PDT_CID_MULIT_MEDIA );
				insert_len += 2;

				//	OUI
				*(uint32_t *) ( pframe + insert_len ) = cpu_to_be32( WPSOUI );
				insert_len += 4;

				//	Sub Category ID
				*(uint16_t *) ( pframe + insert_len ) = cpu_to_be16( WPS_PDT_SCID_MEDIA_SERVER );
				insert_len += 2;


				//	Device Name
				//	Type:
				*(uint16_t *) ( pframe + insert_len ) = cpu_to_be16( WPS_ATTR_DEVICE_NAME );
				insert_len += 2;

				//	Length:
				*(uint16_t *) ( pframe + insert_len ) = cpu_to_be16( pwdinfo->device_name_len );
				insert_len += 2;
...
#ifdef CONFIG_AUTO_AP_MODE
{
	...
	{
		uint8_t RC_OUI[4]={0x00,0xE0,0x4C,0x0A};
		uint8_t RC_INFO[14] = {0};
		//EID[1] + EID_LEN[1] + RC_OUI[4] + MAC[6] + PairingID[2] + ChannelNum[2]
		uint16_t cu_ch = (uint16_t)cur_network->Configuration.DSConfig;
		...
		memcpy(&RC_INFO[10], (uint8_t *)&psta->pid, 2);
		memcpy(&RC_INFO[12], (uint8_t *)&cu_ch, 2);
...
int _issue_probereq(_adapter *padapter, NDIS_802_11_SSID *pssid, uint8_t *da, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame		*pmgntframe;
	struct pkt_attrib		*pattrib;
	...
	*(fctrl) = 0;
	...
	{
		memcpy(pframe, pmlmepriv->probereq_wpsie, pmlmepriv->probereq_wpsie_len);
...
inline void issue_probereq(_adapter *padapter, NDIS_802_11_SSID *pssid, uint8_t *da)
{
	_issue_probereq(padapter, pssid, da, _FALSE);
}

int issue_probereq_ex(_adapter *padapter, NDIS_802_11_SSID *pssid, uint8_t *da,
...
#endif //CONFIG_IOCTL_CFG80211
	{
		if(!rtw_p2p_chk_state(pwdinfo, P2P_STATE_NONE) && !rtw_p2p_chk_state(pwdinfo, P2P_STATE_IDLE))
		{
			...
			*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_be16( pwdinfo->device_name_len );
			...
			//	Length:
			*(uint16_t *) ( p2pie + p2pielen ) = cpu_to_le16( 0x000D );
			p2pielen += 2;
...
//when wait_ack is ture, this function shoule be called at process context
static int _issue_qos_nulldata(_adapter *padapter, unsigned char *da, uint16_t tid, int wait_ack)
{
	int ret = _FAIL;
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
...
//when wait_ms >0 , this function shoule be called at process context
//da == NULL for station mode
int issue_qos_nulldata(_adapter *padapter, unsigned char *da, uint16_t tid, int try_cnt, int wait_ms)
{
	int ret;
	int i = 0;
	uint32_t	 start = rtw_get_current_time();
...
static int _issue_deauth(_adapter *padapter, unsigned char *da, unsigned short reason, uint8_t wait_ack)
{
	struct xmit_frame			*pmgntframe;
	struct pkt_attrib			*pattrib;
	unsigned char					*pframe;
...
int issue_deauth_ex(_adapter *padapter, uint8_t *da, unsigned short reason, int try_cnt,
	int wait_ms)
{
	int ret;
	int i = 0;
...
void issue_action_spct_ch_switch(_adapter *padapter, uint8_t *ra, uint8_t new_ch, uint8_t ch_offset)
{
	_irqL	irqL;
	_list		*plist, *phead;
	struct xmit_frame			*pmgntframe;
...
void site_survey(_adapter *padapter)
{
	...
	else
	{
		...
		if(rtw_p2p_findphase_ex_is_needed(pwdinfo))
		{
			...
			_set_timer( &pwdinfo->find_phase_timer, ( uint32_t	 ) ( ( uint32_t	 ) ( pwdinfo->listen_dwell ) * 100 ) );
		}
		else
#endif //CONFIG_P2P
		{
...
uint8_t chk_ap_is_alive(_adapter *padapter, struct sta_info *psta)
{
	uint8_t ret = _FALSE;
	struct mlme_ext_priv	*pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info	*pmlmeinfo = &(pmlmeext->mlmext_info);
...
static int rtw_auto_ap_start_beacon(_adapter *adapter)
{
	...
	uint8_t *pbuf = NULL;
	uint len;
	uint8_t	supportRate[16];
	int 	sz = 0, rateLen;
	uint8_t *	ie;
	uint8_t	wireless_mode, oper_channel;
	uint8_t ssid[3] = {0}; //hidden ssid
	...
	*(uint16_t *)ie = cpu_to_le16((uint16_t)100);//BCN_INTERVAL=100;
	sz += 2;
	ie += 2;

	//capability info
	*(uint16_t *)ie = 0;
	*(uint16_t *)ie |= cpu_to_le16(cap_ESS);
	*(uint16_t *)ie |= cpu_to_le16(cap_ShortPremble);
	//*(uint16_t *)ie |= cpu_to_le16(cap_Privacy);
	sz += 2;
	ie += 2;
...
//#endif
	...
	if (padapter->registrypriv.wifi_spec) {
		...
		rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_VO, (uint8_t *)(&acparm));
		acparm = 0x005E541C; // VI
		rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_VI, (uint8_t *)(&acparm));
		acparm = 0x0000A525; // BE
		rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_BE, (uint8_t *)(&acparm));
		acparm = 0x0000A549; // BK
		rtw_hal_set_hwreg(padapter, HW_VAR_AC_PARAM_BK, (uint8_t *)(&acparm));
...
uint8_t disconnect_hdl(_adapter *padapter, unsigned char *pbuf)
{
...
uint8_t setauth_hdl(_adapter *padapter, unsigned char *pbuf)
{
...
uint8_t set_tx_beacon_cmd(_adapter* padapter)
{
	struct cmd_obj	*ph2c;
...
{
	uint8_t	*p;
	uint8_t	val8, cur_channel, cur_bwmode, cur_ch_offset, change_band;
	int	ie_len;
	struct registry_priv	*pregpriv = &padapter->registrypriv;
	struct mlme_priv		*pmlmepriv = &(padapter->mlmepriv);
...
uint8_t set_ch_hdl(_adapter *padapter, uint8_t *pbuf)
{
...
uint8_t tdls_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	struct mlme_ext_priv *pmlmeext = &padapter->mlmeextpriv;
	struct mlme_ext_info *pmlmeinfo = &pmlmeext->mlmext_info;
	uint8_t survey_channel, i, min, option;
	...
	switch(option){
		...
		case TDLS_OFF_CH:
			...
			_set_timer(&ptdls_sta->option_timer, (uint32_t)ptdls_sta->ch_switch_time);
			break;
		case TDLS_BASE_CH:
			_cancel_timer_ex(&ptdls_sta->base_ch_timer);
			_cancel_timer_ex(&ptdls_sta->off_ch_timer);
