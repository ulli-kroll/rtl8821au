...
unsigned int on_action_public_default(union recv_frame *precv_frame, uint8_t action)
{
	...
	uint8_t *frame_body = pframe + sizeof(struct rtw_ieee80211_hdr_3addr);
	uint8_t token;
	struct rtl_priv *adapter = precv_frame->u.hdr.adapter;
	int cnt = 0;
	char msg[64];
...
void _linked_info_dump(struct rtl_priv *padapter)
{
	...
	DBG_871X(" OFDM -Alarm DA2(0x%04x),DA4(0x%04x),DA6(0x%04x),DA8(0x%04x)\n",
		rtl_read_word(padapter,0xDA2),rtl_read_word(padapter,0xDA4),rtl_read_word(padapter,0xDA6),rtl_read_word(padapter,0xDA8));
