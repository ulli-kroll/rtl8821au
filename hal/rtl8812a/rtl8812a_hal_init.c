	...
	BOOLEAN	AutoloadFail)
{
	...
	if (!AutoloadFail) {
		if (GetRegAmplifierType2G(Adapter) == 0) {
			...
			if (rtlhal->pa_type_2g == 0xFF && pHalData->LNAType_2G == 0xFF) {
				rtlhal->pa_type_2g = 0;
		...
		if (GetRegAmplifierType5G(Adapter) == 0) {
			...
			if (rtlhal->pa_type_5g == 0xFF && pHalData->LNAType_5G == 0xFF) {
				rtlhal->pa_type_5g = 0;
				pHalData->LNAType_5G = 0;
			}
			pHalData->ExternalPA_5G = ((rtlhal->pa_type_5g & BIT1) && (rtlhal->pa_type_5g & BIT0)) ? 1 : 0;
	...
	BOOLEAN	 AutoloadFail)
{
	...
	if (!AutoloadFail) {
		...
		if (GetRegAmplifierType5G(Adapter) == 0) {
			...
			pHalData->ExternalPA_5G = (rtlhal->pa_type_5g & BIT0) ? 1 : 0;
