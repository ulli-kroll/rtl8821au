...
static void dm_CheckPbcGPIO(struct rtl_priv *rtlpriv)
{
	...
	if (IS_HARDWARE_TYPE_8812(rtlhal)) {
		tmp1byte = usb_read8(rtlpriv, GPIO_IO_SEL);
		tmp1byte |= (HAL_8192C_HW_GPIO_WPS_BIT);
		usb_write8(rtlpriv, GPIO_IO_SEL, tmp1byte);	/* enable GPIO[2] as output mode */

		tmp1byte &= ~(HAL_8192C_HW_GPIO_WPS_BIT);
		usb_write8(rtlpriv,  GPIO_IN, tmp1byte);	/* reset the floating voltage level */

		tmp1byte = usb_read8(rtlpriv, GPIO_IO_SEL);
		tmp1byte &= ~(HAL_8192C_HW_GPIO_WPS_BIT);
		usb_write8(rtlpriv, GPIO_IO_SEL, tmp1byte);	/* enable GPIO[2] as input mode */

		tmp1byte =usb_read8(rtlpriv, GPIO_IN);
		...
		tmp1byte = usb_read8(rtlpriv, GPIO_IO_SEL_8811A);
		tmp1byte |= (BIT4);
		usb_write8(rtlpriv, GPIO_IO_SEL_8811A, tmp1byte);	/* enable GPIO[2] as output mode */

		tmp1byte &= ~(BIT4);
		usb_write8(rtlpriv,  GPIO_IN_8811A, tmp1byte);		/* reset the floating voltage level */

		tmp1byte = usb_read8(rtlpriv, GPIO_IO_SEL_8811A);
		tmp1byte &= ~(BIT4);
		usb_write8(rtlpriv, GPIO_IO_SEL_8811A, tmp1byte);	/* enable GPIO[2] as input mode */

		tmp1byte =usb_read8(rtlpriv, GPIO_IN_8811A);
