...
void rtl8821au_set_hw_reg(struct rtl_priv *padapter, u8 variable, u8 *pval)
{
	...
	case HW_VAR_USB_MODE:
		if (*pval == 1)
			rtw_write8(padapter, REG_OPT_CTRL_8812, 0x4);
		else if (*pval == 2)
			rtw_write8(padapter, REG_OPT_CTRL_8812, 0x8);

		rtw_write8(padapter, REG_SDIO_CTRL_8812, 0x2);
		rtw_write8(padapter, REG_ACLK_MON, 0x1);
		/*
		 ...
		 */
		rtw_write8(padapter, REG_CAL_TIMER+1, 0x40);
		/* rtw_write8(padapter, REG_CAL_TIMER+1, 0x3); */
		rtw_write8(padapter, REG_APS_FSMCO+1, 0x80);
		break;
